...
public class DesktopApplication extends Application {
		...
		public void start(Stage stage) throws Exception {
				...
				webEngine.setUserDataDirectory(Apo.getConfDir());
				...
				loadWorker.stateProperty().addListener(
								(ov, oldState, newState) -> {
										...
										webEngine.executeScript("java.log(console);");
										stage.setTitle("APO Desktop - " + webEngine.getLocation());
										if (newState == Worker.State.SUCCEEDED) {
												nrs = (JSObject) webEngine.executeScript("NRS");
												updateClientState("Desktop Wallet started");
												BlockchainProcessor blockchainProcessor = Apo.getBlockchainProcessor();
												...
												Apo.getTransactionProcessor().addListener((transaction) ->
				...
				stage.getIcons().add(new Image(address + "/img/apo-icon-32x32.png"));
		...
		private static String getUrl() {
				...
				String defaultAccount = Apo.getStringProperty("apo.defaultDesktopAccount");
		...
		private void download(String requestType, Map<String, String> params) {
				...
				if (requestType.equals("downloadTaggedData")) {
						if (taggedData == null && retrieve) {
								try {
										if (Apo.getBlockchainProcessor().restorePrunedTransaction(transactionId) == null) {
