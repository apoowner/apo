...
import apo.db.DbClause;
import apo.db.DbIterator;
import apo.db.DbKey;
import apo.db.EntityDbTable;
...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Apo.getBooleanProperty("apo.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = Apo.getBooleanProperty("apo.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = Apo.getIntProperty("apo.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Apo.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Apo.getEpochTime()), 0, -1, "")) {
    ...
    private final Runnable processTransactionsThread = () -> {
        ...
        try {
            try {
                ...
                } catch (ApoException.ValidationException|RuntimeException e) {
                    peer.blacklist(e);
                }
            } catch (Exception e) {
    ...
    @Override
    public void broadcast(Transaction transaction) throws ApoException.ValidationException {
        BlockchainImpl.getInstance().writeLock();
        try {
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws ApoException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Apo.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    } catch (ApoException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (ApoException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws ApoException.NotValidException {
        if (Apo.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
            return;
        }
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (ApoException.NotCurrentlyValidException ignore) {
            } catch (ApoException.ValidationException|RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new ApoException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws ApoException.ValidationException {
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = Apo.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            throw new ApoException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            throw new ApoException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            throw new ApoException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (Apo.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    throw new ApoException.NotCurrentlyValidException("Blockchain not ready to accept transactions");
                }

                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    throw new ApoException.ExistingTransactionException("Transaction already processed");
                }

                if (! transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        throw new ApoException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new ApoException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (! transaction.applyUnconfirmed()) {
                    throw new ApoException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new ApoException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }
     ...
     * @throws  ApoException.NotValidException    Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws ApoException.NotValidException {
