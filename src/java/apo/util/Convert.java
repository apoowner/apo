...
//        return "APO-" + Crypto.rsEncode(accountId);
        return CURRENCY_NAME + "-" + Crypto.rsEncode(accountId);
    }
    ...
    public static String readString(ByteBuffer buffer, int numBytes, int maxLength) throws ApoException.NotValidException {
        if (numBytes > 3 * maxLength) {
            throw new ApoException.NotValidException("Max parameter length exceeded");
        }
    ...
    public static long parseAPO(String apo) {
        return parseStringFraction(apo, 8, Constants.MAX_BALANCE_APO);
    }
