 ...
 *
 *   java -cp "classes:lib/*:conf" apo.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dapo.runtime.mode=desktop apo.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Apo properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "apo.testDb" : "apo.db";
        String dbType = Apo.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Apo.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Apo.getDbDir(Apo.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Apo.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Apo.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Apo.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "apo.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "apo.mv.db");
            if (!dbFile.exists()) {
