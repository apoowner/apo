...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws ApoException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = Apo.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws ApoException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws ApoException.NotValidException {
        ...
        if (builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Apo.getBlockchain().getHeight());
        ...
        if (builder.signature != null && secretPhrase != null) {
            throw new ApoException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new ApoException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
        ...
        if (bytes == null) {
            try {
                ...
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_APO));
                    buffer.putInt((int) (feeNQT / Constants.ONE_APO));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws ApoException.NotValidException {
        try {
            ...
            if (buffer.hasRemaining()) {
                throw new ApoException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (ApoException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws ApoException.NotValidException {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws ApoException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new ApoException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws ApoException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new ApoException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || Apo.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    @Override
    public void validate() throws ApoException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT <= 0)
                || feeNQT > Constants.MAX_BALANCE_NQT
                || amountNQT < 0
                || amountNQT > Constants.MAX_BALANCE_NQT
                || type == null) {
            throw new ApoException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
        ...
        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new ApoException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new ApoException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new ApoException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new ApoException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion(this.version)) {
                throw new ApoException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new ApoException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }

        if (!validatingAtFinish) {
            int blockchainHeight = Apo.getBlockchain().getHeight();
            long minimumFeeNQT = getMinimumFeeNQT(blockchainHeight);
            if (feeNQT < minimumFeeNQT) {
            	throw new ApoException.NotCurrentlyValidException(String.format("Transaction fee %f " + Constants.COIN_NAME + " less than minimum fee %f " + Constants.COIN_NAME + " at height %d",
                        ((double) feeNQT) / Constants.ONE_APO, ((double) minimumFeeNQT) / Constants.ONE_APO, blockchainHeight));
            }
            if (blockchainHeight > Constants.FXT_BLOCK && ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new ApoException.NotCurrentlyValidException("ecBlockHeight " + ecBlockHeight
    ...
    public long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_APO);
        }
