...
public interface Fee {
    ...
    Fee DEFAULT_FEE = new Fee.ConstantFee(Constants.ONE_APO);
    //Fee DEFAULT_FEE = new Fee.ConstantFee(Constants.TRANSACTION_FEE);
    //long transactionFee = new Long((Apo.getIntProperty("apo.transactionFee", (int) Constants.ONE_APO))/Apo.getIntProperty("apo.reductorFee", 1));
    //long transactionFee = new Long(Apo.getIntProperty("apo.transactionFee", (int) Constants.ONE_APO));
    ...
    final class ConstantFee implements Fee {
        ...
        public long getFee(TransactionImpl transaction, Appendix appendage) {
        	//return fee/Constants.REDUCTOR_FEE < Constants.ONE_APO/Constants.REDUCTOR_FEE ? Constants.ONE_APO/Constants.REDUCTOR_FEE : fee/Constants.REDUCTOR_FEE;
    ...
    abstract class SizeBasedFee implements Fee {
        ...
        public final long getFee(TransactionImpl transaction, Appendix appendage) {
            ...
            if (height <= Constants.NEW_FEE_CALCULATION_BLOCK) {
                constantFee = (this.constantFee < Constants.ONE_APO ? Constants.ONE_APO : this.constantFee)/Constants.REDUCTOR_FEE;
                ...
                return fee < Constants.ONE_APO/Constants.REDUCTOR_FEE ? Constants.ONE_APO/Constants.REDUCTOR_FEE : fee;
            }

            if (this.constantFee == 0) constantFee = 0;
            else constantFee = (this.constantFee < Constants.ONE_APO ? Constants.ONE_APO : this.constantFee)/Constants.REDUCTOR_FEE;
            if (this.feePerSize == 0) feePerSize = 0;
            else feePerSize = (this.feePerSize < Constants.ONE_APO ? Constants.ONE_APO : this.feePerSize)/Constants.REDUCTOR_FEE;

            fee = (Math.addExact(constantFee, Math.multiplyExact((long) (size / unitSize), feePerSize)));

            if (fee != 0) {
                fee = (fee < Constants.ONE_APO ? Constants.ONE_APO : fee)/ Constants.REDUCTOR_FEE;
            }
            return fee;
        }
        ...
        public SizeBasedFee(long constantFee, long feePerSize, int unitSize) {
        	...
        	if (height <= Constants.NEW_FEE_CALCULATION_BLOCK) {
        		this.constantFee = constantFee/Constants.REDUCTOR_FEE < Constants.ONE_APO/Constants.REDUCTOR_FEE ? Constants.ONE_APO/Constants.REDUCTOR_FEE : constantFee/Constants.REDUCTOR_FEE;
        	} else {
	        	if (constantFee == 0) this.constantFee = constantFee;
	        	else this.constantFee = constantFee/Constants.REDUCTOR_FEE < Constants.ONE_APO/Constants.REDUCTOR_FEE ? Constants.ONE_APO/Constants.REDUCTOR_FEE : constantFee/Constants.REDUCTOR_FEE;
        	}
        ...
        public final long getFee(TransactionImpl transaction, Appendix appendage) {
            ...
            //return (Math.addExact(constantFee, Math.multiplyExact((long) (size / unitSize), feePerSize))/Apo.getIntProperty("apo.reductorFee", 1));
