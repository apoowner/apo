...
public final class ShufflingParticipant {
    ...
    void setData(byte[][] data, int timestamp) {
        if (data != null && Apo.getEpochTime() - timestamp < Constants.MAX_PRUNABLE_LIFETIME && getData() == null) {
            shufflingDataTable.insert(new ShufflingData(shufflingId, accountId, data, timestamp, Apo.getBlockchain().getHeight()));
        }
    }
