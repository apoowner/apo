...
import apo.AccountRestrictions;
import apo.Alias;
import apo.Appendix;
import apo.Asset;
import apo.AssetDelete;
import apo.AssetDividend;
import apo.AssetTransfer;
import apo.Attachment;
import apo.Block;
import apo.Constants;
import apo.Currency;
import apo.CurrencyExchangeOffer;
import apo.CurrencyFounder;
import apo.CurrencyTransfer;
import apo.CurrencyType;
import apo.DigitalGoodsStore;
import apo.Exchange;
import apo.ExchangeRequest;
import apo.FundingMonitor;
import apo.Generator;
import apo.HoldingType;
import apo.MonetarySystem;
import apo.Apo;
import apo.Order;
import apo.PhasingPoll;
import apo.PhasingVote;
import apo.Poll;
import apo.PrunableMessage;
import apo.Shuffler;
import apo.Shuffling;
import apo.ShufflingParticipant;
import apo.TaggedData;
import apo.Token;
import apo.Trade;
import apo.Transaction;
import apo.Vote;
import apo.VoteWeighting;
import apo.crypto.Crypto;
import apo.crypto.EncryptedData;
import apo.db.DbIterator;
import apo.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Apo.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceAPO", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceAPO", account.getEffectiveBalanceAPO(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceAPO", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_APO));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.APO) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Apo.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Apo.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Apo.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Apo.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Apo.getBlockchain().getTransaction(entry.getEventId());
