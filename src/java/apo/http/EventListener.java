...
import apo.AccountLedger;
import apo.AccountLedger.LedgerEntry;
import apo.Block;
import apo.BlockchainProcessor;
import apo.Db;
import apo.Apo;
import apo.Transaction;
import apo.TransactionProcessor;
import apo.db.TransactionalDb;
import apo.peer.Peer;
 ...
 * has not been received within apo.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by apo.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Apo.getIntProperty("apo.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Apo.getIntProperty("apo.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Apo.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Apo.getTransactionProcessor();
    ...
    /** Apo event listeners */
    private final List<ApoEventListener> apoEventListeners = new ArrayList<>();
     ...
     *
     * Apo event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Apo event registrations
     ...
     *
     * @param   eventRegistrations      Apo event registrations
     * @throws  EventListenerException  Invalid Apo event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<ApoEventListener> it = apoEventListeners.iterator();
                while (it.hasNext()) {
                    ApoEventListener listener = it.next();
                ...
                if (addListener) {
                    ApoEventListener listener = new ApoEventListener(event);
                    listener.addListener();
                    apoEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            apoEventListeners.forEach(ApoEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Apo event listener
     */
    private class ApoEventListener {

        /** Event handler */
        private final ApoEventHandler eventHandler;

        /**
         * Create the Apo event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public ApoEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Apo listener for this event
         */
        ...
        /**
         * Remove the Apo listener for this event
         */
        ...
        /**
         * Return the hash code for this Apo event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Apo events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof ApoEventListener) &&
                    eventHandler.equals(((ApoEventListener)obj).eventHandler));
        }

        /**
         * Apo listener event handler
         */
        private abstract class ApoEventHandler {
            ...
            /**
             * Create the Apo event handler
             *
             * @param   eventRegistration   Event registration
             */
            public ApoEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof ApoEventHandler) &&
                                        owner == ((ApoEventHandler)obj).owner &&
                                        accountId == ((ApoEventHandler)obj).accountId &&
                                        event == ((ApoEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends ApoEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends ApoEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends ApoEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends ApoEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Apo listener event
        ...
        /**
         * Return the Apo listener event
         *
         * @return                  Apo listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
