...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Apo.getBooleanProperty("apo.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = Apo.getIntProperty("apo.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Apo.getStringProperty("apo.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Apo.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, Apo.getEpochTime() + blacklistingPeriod);
