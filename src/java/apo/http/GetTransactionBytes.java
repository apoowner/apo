...
import static apo.http.JSONResponses.INCORRECT_TRANSACTION;
import static apo.http.JSONResponses.MISSING_TRANSACTION;
import static apo.http.JSONResponses.UNKNOWN_TRANSACTION;
...
public final class GetTransactionBytes extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        transaction = Apo.getBlockchain().getTransaction(transactionId);
        JSONObject response = new JSONObject();
        if (transaction == null) {
            transaction = Apo.getTransactionProcessor().getUnconfirmedTransaction(transactionId);
            if (transaction == null) {
                return UNKNOWN_TRANSACTION;
            }
        } else {
            response.put("confirmations", Apo.getBlockchain().getHeight() - transaction.getHeight());
        }
