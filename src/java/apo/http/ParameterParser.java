...
import apo.Currency;
import apo.CurrencyBuyOffer;
import apo.CurrencySellOffer;
...
import static apo.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, Apo.getBlockchain().getHeight(), false);
    }
    ...
    public static int getHeight(HttpServletRequest req) throws ParameterException {
        ...
        if (heightValue != null) {
            try {
                ...
                if (height < 0 || height > Apo.getBlockchain().getHeight()) {
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.APO);
        if (holdingType == HoldingType.APO && holdingId != 0) {
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return Apo.newTransactionBuilder(json);
            } catch (ApoException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return Apo.newTransactionBuilder(bytes, prunableAttachments);
            } catch (ApoException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, ApoException.NotValidException {
