...
import static apo.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static apo.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static apo.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = Apo.getStringProperty("apo.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Apo.getIntProperty("apo.maxAPIRecords");
    static final boolean enableAPIUPnP = Apo.getBooleanProperty("apo.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Apo.getIntProperty("apo.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Apo.getBooleanProperty("apo.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(Apo.getStringListProperty("apo.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Apo.getStringListProperty("apo.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Apo.getStringListProperty("apo.allowedBotHosts");
        ...
        boolean enableAPIServer = Apo.getBooleanProperty("apo.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Apo.getIntProperty("apo.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Apo.getIntProperty("apo.apiServerSSLPort");
            final String host = Apo.getStringProperty("apo.apiServerHost");
            disableAdminPassword = Apo.getBooleanProperty("apo.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Apo.getBooleanProperty("apo.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Apo.getUserHomeDir()).resolve(Paths.get(Apo.getStringProperty("apo.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Apo.getStringProperty("apo.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Apo.getStringProperty("apo.keyStoreType"));
                List<String> ciphers = Apo.getStringListProperty("apo.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Apo.getStringProperty("apo.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Apo.getStringProperty("apo.apiWelcomeFile")});
            }

            String javadocResourceBase = Apo.getStringProperty("apo.javadocResourceBase");
            ...
            //ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/apo");
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/"+Constants.COIN_NAME.toLowerCase());
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Apo.getIntProperty("apo.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            //servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/apo-proxy");
            ...
            if (!Apo.getBooleanProperty("apo.enableAPIServerGZIPFilter")) {
            	//gzipHandler.setExcludedPaths("/apo", "/apo-proxy");
                gzipHandler.setExcludedPaths("/"+Constants.COIN_NAME.toLowerCase(), "/"+Constants.COIN_NAME.toLowerCase()+"-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(apo.peer.Peers.MIN_COMPRESS_SIZE);
            apiHandler.setGzipHandler(gzipHandler);

            if (Apo.getBooleanProperty("apo.enableApiTestUI")) {
            ...
            if (Apo.getBooleanProperty("apo.apiFrameOptionsSameOrigin")) {
