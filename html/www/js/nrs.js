...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.mobileSettings = {
        ...
        is_testnet: false,
		// remote_node_address: "a.coin.cards",
		remote_node_address: "de.winacc.com",
		remote_node_port: "",
    ...
    function initMobile() {
        ...
        promise.then(function() {
			...
			var loadConstantsPromise = new Promise(function(resolve) {
				console.log("Mobile load server constants");
    ...
    function initImpl() {
		var loadConstantsPromise = new Promise(function(resolve) {
			...
			NRS.loadServerConstants(resolve);
			// console.log("END load server constants");
		});
		loadConstantsPromise.then(function() {
			...
			getStatePromise.then(function() {
				...
				$("#dgs_search_account_center").mask("APO-****-****-****-*****");
...
NRS.addPagination = function () {
	...
	NRS.getRequestPath = function (noProxy) {
		//suse 03/01/2017 - choose the API point
		// console.log("NRS.getRequestPath");
		// var url = NRS.getRemoteNodeUrl();
		var url = NRS.getRemoteNodeUrl();
		if (!NRS.state.apiProxy || noProxy) {
			// return url + "/apo";
			return url + "/" + NRS.apiEndpoint;
		} else {
			// return url + "/apo-proxy";
			return url + "/" + NRS.apiEndpoint + "-proxy";
		}
	};
	...
	NRS.getAccountInfo = function(firstRun, callback, isAccountSwitch) {
		NRS.sendRequest("getAccount", {
			...
			if (firstRun) {
		    	...
		    	*/
				//suse 12/04/2017
				//remove option from settings area
				$("h3[data-i18n=\"theme_settings\"]").parent("div").parent("div").parent("div").remove();
				$("h3[data-i18n=\"exchange_settings\"]").parent("div").parent("div").parent("div").remove();
				$("label[data-i18n=\"enable_marketplace_section\"]").parent("div").remove();
				$("label[data-i18n=\"enable_exchange_section\"]").parent("div").remove();
				$("label[data-i18n=\"enable_news_section\"]").parent("div").remove();
				$("label[data-i18n=\"enable_plugins_setting\"]").parent("div").remove();
				$("label[data-i18n=\"show_console_log_button\"]").parent("div").remove();
				//$("label[data-i18n=\"admin_password\"]").parent("div").remove();
				$("#messages_decrypt_remember_password").parent("div").remove();
				//suse 26/04/2017
				//Remove Total Available in My Assets
				$("th[data-i18n=\"total_available\"]").remove();
				//Remove Total Available in Assets page
				$("#asset_quantity").remove();
				//Remove the Unconfirmed transaction Indicator from the top menu
				$("#unconfirmed_notification_menu").parent("li").remove();
				...
				if (NRS.defaultModules['asset_exchange'] === false) {
					$("#account_assets_balance").parents(".optional_dashboard_tile").remove();
					NRS.logConsole('account_assets_balance.optional_dashboard_tile REMOVED');
					$("#user_info_assets").remove();
					NRS.logConsole('user_info_assets REMOVED');
					$("#user_info_trade_history").remove();
					NRS.logConsole('user_info_trade_history REMOVED');
					$("li[data-i18n=\"[data-content]approve_modal_by_asset_tab_help\"]").remove();
					NRS.logConsole('li[data-i18n=\"[data-content]approve_modal_by_asset_tab_help\"] REMOVED');
					$("span[data-i18n=\"asset_balance\"]").parent("label").remove();
					$("option[data-i18n=\"vote_by_asset\"]").remove();
					NRS.disableTransacionType[2] = true;
					
				}
				if (NRS.defaultModules['send_token'] === false) {
					$("#header_send_money").remove();
					NRS.logConsole('header_send_money REMOVEDD');
					$("#user_info_ledger").remove();
					NRS.logConsole('user_info_ledger REMOVED');
					$("button[data-i18n='send_apo']").remove();
					NRS.logConsole('button[data-i18n=\'send_apo\'] REMOVED');
					NRS.disableTransacionType[0] = true;
				}
				if (NRS.defaultModules['monetary_system'] === false) {
					$("#account_currencies_balance").parents(".optional_dashboard_tile").remove();
					NRS.logConsole('account_currencies_balance.optional_dashboard_tile REMOVED');
					$("#transaction_info_modal_transfer_currency").remove();
					NRS.logConsole('transaction_info_modal_transfer_currency REMOVED');
					$("#user_info_currencies").remove();
					NRS.logConsole('user_info_currencies REMOVED');
					$("button[data-i18n='send_currency']").remove();
					NRS.logConsole('button[data-i18n=\'send_currency\'] REMOVED');
					$("li[data-i18n=\"[data-content]approve_modal_by_currency_tab_help\"]").remove();
					NRS.logConsole('li[data-i18n=\"[data-content]approve_modal_by_currency_tab_help\"] REMOVED');
					$("span[data-i18n=\"currency_balance\"]").parent("label").remove();
					$("option[data-i18n=\"vote_by_currency\"]").remove();
					NRS.disableTransacionType[5] = true;
				}
				if (NRS.defaultModules['polls'] === false) {
				}
				if (NRS.defaultModules['data_cloud'] === false) {
					NRS.disableTransacionType[6] = true;
				}
				if (NRS.defaultModules['marketplace'] === false) {
					$("#account_purchase_count").parents(".optional_dashboard_tile").remove();
					NRS.logConsole('account_purchase_count.optional_dashboard_tile REMOVED');
					$("#account_pending_sale_count").parents(".optional_dashboard_tile").remove();
					NRS.logConsole('account_pending_sale_count.optional_dashboard_tile REMOVED');
					$("#user_info_marketplace").remove();
					NRS.logConsole('user_info_marketplace REMOVED');
					NRS.disableTransacionType[3] = true;
				}
				if (NRS.defaultModules['shuffling'] === false) {
					NRS.disableTransacionType[7] = true;
				}
				if (NRS.defaultModules['messages'] === false) {
					$("#account_message_count").parents(".optional_dashboard_tile").remove();
					NRS.logConsole('account_message_count.optional_dashboard_tile REMOVED');
					$("#header_send_message").remove();
					NRS.logConsole('header_send_message REMOVED');
					$("#notification_menu").remove();
					NRS.logConsole('notification_menu REMOVED');
					$("#unconfirmed_notification_menu").remove();
					NRS.logConsole('unconfirmed_notification_menu REMOVED');
					$("button[data-i18n='send_a_message']").remove();
					NRS.logConsole('button[data-i18n=\'send_a_message\'] REMOVED');
				}
				if (NRS.defaultModules['aliases'] === false) {
					$("#account_alias_count").parents(".optional_dashboard_tile").remove();
					NRS.logConsole('account_alias_count.optional_dashboard_tile REMOVED');
					$("#user_info_aliases").remove();
					NRS.logConsole('user_info_aliases REMOVED');
				}
				if (NRS.defaultModules['polls'] === false && NRS.defaultModules['messages'] === false && NRS.defaultModules['aliases'] === false) {
					NRS.disableTransacionType[1] = true;
				}
				if (NRS.defaultModules['plugins'] === false) {
					$("a[data-page='plugins_overview']").parent("li").prev('li').remove();
					$("a[data-page='plugins_overview']").parent("li").remove();
					//$("#plugin_menu_item").remove();
					//NRS.logConsole('plugin_menu_item REMOVED');
					//$("#plugin_menu_divider").remove();
					//NRS.logConsole('plugin_menu_divider REMOVED');
				}
				if (NRS.defaultModules['issue_asset'] === false) {
					$("a[data-target='#issue_asset_modal']").parent("li").remove();
				}
				if (NRS.defaultModules['add_asset_button'] === false) {
					$("#asset_exchange_add_asset_bookmark").parent("div").remove();
				}
				if (NRS.defaultModules['transactions_notification'] === false) {
					$("#notification_menu").parent("li").remove();
				}
        ...
        if (NRS.accountInfo.lessors) {
			...
			for (var i = 0; i < NRS.accountInfo.lessorsRS.length; i++) {
				...
				rows += "<tr>" +
					...
					"<td>" + NRS.escapeRespStr(lessorInfo.effectiveBalanceAPO) + "</td>" +
		...
		if (NRS.accountInfo.accountControls && $.inArray('PHASING_ONLY', NRS.accountInfo.accountControls) > -1) {
			NRS.sendRequest("getPhasingOnlyControl", {
				...
				if (response && response.votingModel >= 0) {
					...
					if (response.maxFees) {
						data.maximum_fees = NRS.convertToAPO(response.maxFees);
					}
