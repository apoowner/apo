...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			labelText: "Amount APO",
			labelI18n: "amount_apo",
			helpI18n: "approve_transaction_amount_help",
			inputName: "phasingQuorumAPO",
			addonText: "APO",
			addonI18n: "apo_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_apo', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceAPO';
		context['addonText'] = 'APO';
		context['addonI18n'] = 'apo_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_apo', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
    ...
    NRS.forms.signTransaction = function() {
		...
		if (data.unsignedTransactionBytes && !data.validate) {
			...
			if (NRS.getAccountId(secretPhrase) == NRS.account || isOffline) {
				try {
					var signature = NRS.signBytes(data.unsignedTransactionBytes, converters.stringToHexString(NRS.transformSecretPhrase(secretPhrase)));
