...
var NRS = (function(NRS, $) {
	...
	NRS.tagged_data_search_main = function(callback) {
		...
		$(".tagged_data_search input[name=q]").val("").trigger("unmask").mask("APO-****-****-****-*****");
	...
	NRS.setup.tagged_data_search = function() {
		//suse 12/01/2017
		if (NRS.defaultModules['data_cloud'] === false) return;
		///suse
    ...
    $("#extend_data_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getTransaction", {
            ...
            var fee = NRS.convertToAPO(NRS.escapeRespStr(response.feeNQT));
            $('#extend_data_fee').val(fee);
            $('#extend_data_fee_label').html(String(fee) + " APO");
        })
    });
