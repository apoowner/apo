...
var NRS = (function (NRS, $, undefined) {
    ...
    search.on("submit", function (e, data) {
        ...
        $("#buy_currency_with_apo").html($.t("buy_currency_param", { currency: currencyCode}));
        $("#buy_currency_offers").html($.t("offers_to_buy_currency_param", { currency: currencyCode}));
        $("#sell_currency_with_apo").html($.t("sell_currency_param", { currency: currencyCode}));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
                    $("#ms_your_apo_balance").html("0");
                } else {
                    $("#ms_your_apo_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
                }
                NRS.pageLoaded();
                callback(null);
            }
        ...
        NRS.sendRequest("getCurrencyFounders", {
            ...
            if (response.founders && response.founders.length) {
                ...
                for (i = 0; i < response.founders.length; i++) {
                    ...
                    "<td>" + NRS.convertToAPO(amountPerUnitNQT) + "</td>" +
                    "<td>" + NRS.convertToAPO(amountPerUnitNQT.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
            ...
            rows += "<tr>" +
            "<td><b>Totals</b></td>" +
            "<td>" + NRS.convertToAPO(totalAmountReserved) + "</td>" +
            "<td>" + NRS.convertToAPO(totalAmountReserved.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
    ...
    $("#currency_order_modal").on("show.bs.modal", function (e) {
        ...
        var totalAPO = NRS.formatAmount(totalNQT, false, true);
        ...
        $("#currency_order_total").html(totalAPO + " APO");
    ...
    unitFields.on("change", function() {
        ...
        NRS.sendRequest("getAvailableTo" + NRS.initialCaps(orderType), {
            ...
            rateField.val(rate);
            var amount = NRS.convertToAPO(response.amountNQT);
    ...
    $("#transfer_currency_modal").on("show.bs.modal", function (e) {
        console.log('nrs.monetarysystem.js show.bs.modal');
        var $invoker = $(e.relatedTarget);
    ...
    NRS.setup.currencies = function () {
        //suse 12/01/2017
        if (NRS.defaultModules['monetary_system'] === false) return;
        ///suse
    ...
    $("#reserve_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency+", {
            ...
            if (response && !response.errorDescription) {
                ...
                $("#reserve_currency_minReserve_text").html(NRS.formatQuantity(NRS.convertToAPO(minReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
                ...
                $("#reserve_currency_currentReserve_text").html(NRS.formatQuantity(NRS.convertToAPO(currentReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
    ...
    reserveCurrencyAmount.blur(function () {
        ...
        var roundUnitAmountNQT = NRS.convertToNQT(NRS.amountToPrecision(NRS.convertToAPO(unitAmountNQT), decimals));
        $("#reserve_currency_total").val(NRS.convertToAPO(roundUnitAmountNQT));
        reserveCurrencyAmount.val(NRS.convertToAPO(new BigInteger(roundUnitAmountNQT).multiply(new BigInteger(resSupply)).toString()));
    });
    ...
    $("#claim_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency", {
            ...
            $("#claimRate").html(NRS.formatAmount(currentReservePerUnitNQT) + " [APO/" + currencyCode + "]");
        });
