...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.pages.asset_exchange = function (callback) {
        ...
        NRS.storageSelect("assets", null, function (error, assets) {
            ...
            /*for (var dfltAsset in NRS.defaultAsset) {
            	//console.log('ExistingAsset '+NRS.defaultAsset[dfltAsset]+' = '+existingAsset[NRS.defaultAsset[dfltAsset]])
            	if (existingAsset[NRS.defaultAsset[dfltAsset]] != 1) {
                	//console.log("Add default asset ID=" + dfltAsset + " ID="+NRS.defaultAsset[dfltAsset]);
        	        NRS.sendRequest("getAsset", {
        	                "asset": NRS.defaultAsset[dfltAsset]
        	            }, function (response) {
        	            	//console.log(response);
        	            	//console.log('String(NRS.accountRS).escapeHTML()='+String(NRS.accountRS).escapeHTML());
        	            	if (response.accountRS != String(NRS.accountRS).escapeHTML()) {
        	            		NRS.saveAssetBookmarks(new Array(response), NRS.forms.addAssetBookmarkComplete);
        	            	}
        	            });
            	}
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^APO\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^APO\-/i.test(data.id)) {
    ...
    NRS.loadAsset = function (asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_apo").html($.t("sell_asset_for_apo", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
            $("#buy_asset_with_apo").html($.t("buy_asset_with_apo", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
        ...
        if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
            $("#your_apo_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_apo_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
    ...
    $("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function (e) {
        ...
        try {
            ...
            $("#" + type + "_asset_total").val(NRS.convertToAPO(totalNQT));
        } catch (err) {
            return;
        }
    ...
    $("#sell_automatic_price, #buy_automatic_price").on("click", function () {
        try {
            ...
            if (price.cmp(new Big("0")) <= 0) {
                ...
                assetPrice.val(NRS.convertToAPO(price.toString()));
            }
            ...
            assetTotal.val(NRS.convertToAPO(total.toString()));
            assetTotal.css({
                "background": "",
                "color": ""
            });
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var priceNQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToNQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));
            var totalAPO = NRS.formatAmount(NRS.calculateOrderTotalNQT(quantityQNT, priceNQT, currentAsset.decimals), false, true);
        } catch (err) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "apo": NRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "apo": NRS.formatAmount(priceNQTPerWholeQNT, false, true),
                "total_apo": totalAPO
            });
        } else {
        ...
        $("#asset_order_total").html(totalAPO + " APO");
    ...
    NRS.setup.asset_exchange = function () {
        //suse 12/01/2017
        if (NRS.defaultModules['asset_exchange'] === false) return;
        ///suse
