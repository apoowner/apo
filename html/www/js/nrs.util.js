...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		...
		return NRS.convertToAPO(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};
    ...
    NRS.calculateOrderTotal = function (quantityQNT, priceNQT) {
        return NRS.convertToAPO(calculateOrderTotalImpl(quantityQNT, priceNQT));
    };
    ...
    NRS.convertToAPO = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
    ...
    NRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = NRS.convertToAPO(amount, true);
        ...
        if (NRS.settings) {
            ...
            var maxLength = parseInt(NRS.settings.max_apo_decimals) + offset;
    ...
    NRS.createInfoTable = function (data, fixed) {
		...
		for (var key in data) {
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                value = NRS.formatAmount(new BigInteger(String(value))) + " APO";
    ...
    $("#offcanvas_toggle").on("click", function (e) {
		...
		if ($(window).width() <= 992) {
            ...
            //alert("rowOffCanvas.hasClass('active')="+rowOffCanvas.hasClass('active'));
            //if (rowOffCanvas.hasClass('active')) logo.css('margin-left','200px');
            //else logo.css('margin-left','40px');
            //alert("logo.style.marginLeft="+logo.css('margin-left'));
            if (rowOffCanvas.hasClass('active')) logo.css('margin-left','220px');
            else logo.css('margin-left','');
		} else {
			//Else, enable content streching
            leftSide.toggleClass("collapse-left");
            rightSide.toggleClass("strech");
            if (leftSide.hasClass('collapse-left')) logo.css('margin-left','40px');
            else logo.css('margin-left','');
		}
    ...
    $.fn.tree = function () {
      ...
      console.log('fn.tree called', menu);
    ...
    NRS.generateToken = function(message, secretPhrase) {
        ...
        token = token.concat(tsb, converters.hexStringToByteArray(NRS.signBytes(converters.byteArrayToHexString(messageBytes), secretPhrase !== undefined ? converters.stringToHexString(NRS.transformSecretPhrase(secretPhrase)) : undefined)));
