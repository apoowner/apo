...
var NRS = (function(NRS, $, undefined) {
				...
				if (responsePoll.transaction) {
					NRS.sendRequest("getPhasingPollVote", {
						...
						if (vm == 1) {
							$popoverTypeTR.find("td:first").html($.t('accounts', 'Accounts') + ":");
							$popoverTypeTR.find("td:last").html(String(attachment.phasingWhitelist ? attachment.phasingWhitelist.length : ""));
							votesFormatted = NRS.convertToAPO(responsePoll.result) + " / " + NRS.convertToAPO(attachment.phasingQuorum) + " APO";
							$popoverVotesTR.find("td:last").html(votesFormatted);
						}
						if (mbModel == 1) {
							if (minBalance > 0) {
								minBalanceFormatted = NRS.convertToAPO(minBalance) + " APO";
    ...
    NRS.getTransactionRowHTML = function(t, actions, decimals) {
    	...
    	console.log('t.subtype="'+t.subtype+'"');
    	console.log(NRS.transactionTypes);
    	console.log("NRS.transactionTypes[t.type]['subTypes'][t.subtype]['i18nKeyTitle']"+NRS.transactionTypes[t.type]['subTypes'][t.subtype]['i18nKeyTitle']);
    	if (t.type == 1 && t.subtype == 1) {
    		t.subtype = 0;
    	}
    	console.log('t.type='+t.type);
    	console.log('t.subtype='+t.subtype);
	...
	NRS.buildTransactionsTypeNavi = function() {
		...
		$.each(NRS.transactionTypes, function(typeIndex, typeDict) {
			...
			if (NRS.disableTransacionType[typeIndex] != true) $('#transactions_type_navi').append(html);
		});
