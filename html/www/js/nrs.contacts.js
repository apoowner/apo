...
var NRS = (function(NRS, $) {
			...
			if (contacts && contacts.length) {
				...
				$.each(contacts, function (index, contact) {
					...
					rows += "<tr>" +
						...
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("send_apo") + "</a>&nbsp;" +
	...
	NRS.forms.addContact = function($modal) {
		...
		if (/^\d+$/.test(data.name) || /^APO\-/i.test(data.name)) {
			return {
		...
		var address;
		if (/^APO\-/i.test(data.account_id)) {
			data.account_rs = data.account_id;
			address = new ApoAddress();
	...
	$("#update_contact_modal").on("show.bs.modal", function(e) {
		...
		if (!contactId && NRS.selectedContext) {
			...
			var dbKey = (/^APO\-/i.test(accountId) ? "accountRS" : "account");
			var dbQuery = {};
