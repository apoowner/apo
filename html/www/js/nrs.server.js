...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var winFields = [
                ...
                ["priceAPO", "priceNQT"],
                ["refundAPO", "refundNQT"],
                ["discountAPO", "discountNQT"],
                ["phasingQuorumAPO", "phasingQuorum"],
                ["phasingMinBalanceAPO", "phasingMinBalance"],
                ["controlQuorumAPO", "controlQuorum"],
                ["controlMinBalanceAPO", "controlMinBalance"],
                ["controlMaxFeesAPO", "controlMaxFees"],
                ["minBalanceAPO", "minBalance"],
                ["shufflingAmountAPO", "amount"],
                ["monitorAmountAPO", "amount"],
                ["monitorThresholdAPO", "threshold"]
            ];

            for (i = 0; i < winFields.length; i++) {
                var winField = winFields[i][0];
                var nqtField = winFields[i][1];
                if (winField in data) {
                    data[nqtField] = NRS.convertToNQT(data[winField]);
                    delete data[winField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToAPO(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (apoAddress.set(accountId)) {
                accountRS = apoAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/" +  NRS.apiEndpoint;
        } else {
        ...
        $.ajax({
            ...
            if (secretPhrase && response.unsignedTransactionBytes && !data.doNotSign && !response.errorCode && !response.error) {
                ...
                var signature = NRS.signBytes(response.unsignedTransactionBytes, converters.stringToHexString(NRS.transformSecretPhrase(secretPhrase)));
