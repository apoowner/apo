...
var NRS = (function(NRS, $) {
	...
	NRS.setup.messages = function() {
		//suse 12/01/2017
		if (NRS.defaultModules['messages'] === false) return;
		///suse
    ...
    */
	...
	NRS.forms.decryptMessages = function($modal) {
		...
		try {
			...
			//suse
			//console.log('messagesToDecrypt');
			//console.log(messagesToDecrypt);
			//suse force setDecriptionPassword
    ...
    $("#messages_decrypt_modal").on("show.bs.modal", function () {
		$("#messages_decrypt_password, #messages_decrypt_remember_password, #messages_decrypt_shared_key, " +
            "#decrypt_note_form_password, #decrypt_note_remember_password, #decrypt_note_form_shared_key").prop('disabled', false);
    });

    $("#transaction_info_modal").on("show.bs.modal", function (e) {  
		$("#messages_decrypt_password, #messages_decrypt_remember_password, #messages_decrypt_shared_key, " +
        "#decrypt_note_form_password, #decrypt_note_remember_password, #decrypt_note_form_shared_key").prop('disabled', false);
		...
		var sharedKey = $invoker.data("sharedkey");
        $("#shared_key_text").val(sharedKey);
		var transactionId = $invoker.data("transaction");
        $("#shared_key_transaction").html(NRS.getTransactionLink(transactionId));
