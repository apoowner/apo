...
var NRS = (function(NRS, $) {
	var depositButtonLoaded = 0;
    var DEPOSIT_ADDRESSES_KEY = "shapeshift.depositAddresses.";
    var SUPPORTED_COINS = {};

    var coinToPair = function (op, coin) {
        return (op == "buy") ? "APO_" + coin : coin + "_APO";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("APO_") == 0) {
            return pair.substring("APO_".length);
        }
        if (pair.indexOf("_APO") == pair.length - "_APO".length) {
            return pair.substring(0, pair.indexOf("_APO"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_apo");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "APO" && String(transaction.curOut).escapeHTML() != "APO") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderApoLimit() {
        apiCall('limit/apo_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_apo_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'APO' && coin['status'] == 'available') {
    ...
    $("#deposit_coin_modal").on("show.bs.modal", function (e) {
        ...
        var agentEmail= "admin@apolo-cc.org";
        var coin = invoker.attr("coin-type");
        var title = invoker.text();
        $("#deposit_coin_modal_title").html(title);
        //$("#exchange_deposit_mail").html($.t("exchange_deposit_mail", { coin: coin, agentEmail: "apo_agent@email.com" }));
    ...
    NRS.pages.exchange = function() {
        ...
        //renderRecentTable();
        //exchangePageHeader.remove();
        exchangePageContent.remove();
        exchange_deposit_coin.show();
        //var coin = 'BTC';
        //var agentEmail = 'apo_agent@email.com';
        //$("#exchange_deposit_mail").html($.t("exchange_deposit_mail", { coin: "BTC", agentEmail: "apo_agent@email.com" }));
        loadDepositCoins();
        //<a class="btn btn-default" href="#" id="exchange_deposit_btc" data-toggle="modal" data-i18n="exchange_deposit_btc">exchange_deposit_btc</a>
    ...
    NRS.getFundAccountLink = function() {
        //return "<div class='callout callout-danger'>" +
            ...
            //"data-pair='BTC_APO'>" + $.t("fund_account_message") + "</a>";
            return getDepositCoinButton().join(" ");
    };
    ...
    $("#m_shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#m_shape_shift_buy_title").html($.t("exchange_apo_to_coin_shift", { coin: coin }));
        $("#m_shape_shift_buy_min").val(invoker.data("min"));
        $("#m_shape_shift_buy_min_coin").html("APO");
        $("#m_shape_shift_buy_max").val(invoker.data("max"));
        $("#m_shape_shift_buy_max_coin").html("APO");
        $("#m_shape_shift_buy_rate").val(invoker.data("rate"));
        $("#m_shape_shift_buy_rate_text").html("APO/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_apo_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("APO/" + coin);
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                if (amount != data.success.withdrawalAmount) {
                    NRS.showModalError("amount returned from APO exchange " + data.success.withdrawalAmount +
                    " differs from requested amount " + amount, modal);
                    return;
                }
                if (withdrawal != data.success.withdrawal) {
                    NRS.showModalError("withdrawal address returned from APO exchange " + data.success.withdrawal +
                ...
                // add 1 APO fee to make sure the net amount is what requested by shape shift
    ...
    $("#m_shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_shape_shift_sell_title").html($.t("exchange_coin_to_apo_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#m_shape_shift_sell_rate_text").html(coin + "/APO");
                $("#m_shape_shift_sell_fee").val(data.fee);
                $("#m_shape_shift_sell_fee_coin").html("APO");
    ...
    $("#m_send_amount_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_apo_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("APO/" + coin);
        $("#m_send_amount_sell_fee_coin").html("APO");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("APO");
