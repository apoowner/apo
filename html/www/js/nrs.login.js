...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedWinAccounts") && NRS.getStrItem("savedWinAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedWinAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (type == "account") {
            ...
            NRS.scanQRCode(readerId, function(text) {
                var apoAddress = new ApoAddress();
                if (apoAddress.set(text)) {
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedWinAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedWinAccounts');
		} else {
			NRS.setStrItem("savedWinAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedWinAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedWinAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedWinAccounts", accountsStr + account + ";");
    }
		...
		NRS.sendRequest("getBlockchainStatus", {}, function(response) {
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				if (!response.errorCode) {
					...
					if (isPassphraseLogin) {
                        NRS.publicKey = NRS.getPublicKey(converters.stringToHexString(NRS.transformSecretPhrase(id)));
                    } else {
          ...
          // barton 07-04-2017
          if(NRS.custom && NRS.custom.setup) {
            NRS.custom.setup();
          }
					...
					if (NRS.getStrItem("savedWinAccounts") && NRS.getStrItem("savedWinAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedWinAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Apo passphrase.";
			html += "</p>";
			...
			html += "you can loose your APO! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of APO now or in the future.";
			html += "</p>";
			html += "</div>";
