...
var NRS = (function (NRS, $) {
		...
		if (!options.sharedKey) {
			...
			if (!options.publicKey) {
				...
				try {
					...
					if (!apoAddress.set(options.account)) {
						throw {
  ...
  NRS.encryptNote = function(message, options, secretPhrase) {
	...
	NRS.tryToDecryptMessage = function(message) {
		//suse
		//console.log('tryToDecryptMessage');
		if (_decryptedTransactions && _decryptedTransactions[message.transaction]) {
			//suse
			//console.log('if (_decryptedTransactions && _decryptedTransactions[message.transaction])');
			if (_decryptedTransactions[message.transaction].encryptedMessage) {
				//suse
				//console.log('if (_decryptedTransactions[message.transaction].encryptedMessage)');
		...
		try {
			...
			if (!message.attachment.encryptedMessage.data) {
				//suse
				//console.log('if (!message.attachment.encryptedMessage.data)');
	...
	NRS.decryptAllMessages = function(messages, password, sharedKey) {
		...
		//suse
		//console.log('decryptAllMessages');
		...
		for (var i = 0; i < messages.length; i++) {
			...
			//console.log(message);
			//console.log('!_decryptedTransactions[message.transaction]='+_decryptedTransactions[message.transaction]);
			//console.log('_encryptedNote');
			//console.log(_encryptedNote);
			if (message.attachment.encryptedMessage && !_decryptedTransactions[message.transaction]) {
				try {
					...
					var options = {};
					//suse
					//console.log('START if (useSharedKey)');
					if (useSharedKey) {
						options.sharedKey = converters.hexStringToByteArray(sharedKey);
					} else {
					...
					//suse
					//console.log('END if (useSharedKey)');
					//console.log('_encryptedNote');
					//console.log(_encryptedNote);
					//console.log('START if (_encryptedNote.transaction.goodsIsText)='+_encryptedNote.transaction.goodsIsText);
					...
					//suse
					//console.log('END if (_encryptedNote.transaction.goodsIsText)');
